...
resolve_path( pathresolver *table, uint64_t in_dpid, uint16_t in_port,
  ...
  build_topology_table( table );
	...
	/* Apply the WSP routing */
	return WSP_routing( table->node_table, in_dpid, in_port, out_dpid, out_port );
...
// Added
unsigned int
find_min_r(hash_table *node_table, dlist_element *dlist) {
	node *n_next;
	node *n_prev;

	edge *e;

	unsigned int min_r = UINT32_MAX;

	pathresolver_hop *hh_next;
	pathresolver_hop *hh_prev;

	while( dlist->prev != NULL ) {
		hh_next = ( pathresolver_hop * )dlist->data;
		hh_prev = ( pathresolver_hop * )dlist->prev->data;

		n_next = lookup_node( node_table, hh_next->dpid );
		n_prev = lookup_node( node_table, hh_prev->dpid );

		e = lookup_edge( n_prev, n_next );

		if( e->cost < min_r )	min_r = e->cost;
...
static hash_table *
create_dlist_table() {
	return create_hash( comp_dlist, hash_dlist );
}

static bool
comp_dlist( const void *x0, const void *y0 ) {
	const dlist_element *x = x0;
	const dlist_element *y = y0;

	pathresolver_hop *h_x;
	pathresolver_hop *h_y;
//	dlist_element *tmp_x;
//	dlist_element *tmp_y;
...
//	tmp_y = y;
	...
	while( x != NULL && y != NULL ) {
		h_x = ( pathresolver_hop * )x->data;
		h_y = ( pathresolver_hop * )y->data;

		if( h_x->dpid != h_y->dpid )	return false;
...
static unsigned int
hash_dlist( const void *key0 ) {
	const dlist_element *key = key0;

	pathresolver_hop *h = ( pathresolver_hop * )key->data;

	return hash_datapath_id( &h->dpid );
}

// Update hash_table data. If from data not found then add the data.
static void
update( hash_table *dlist_table, dlist_element *from, dlist_element *to) {
	pathresolver_hop *h_from = ( pathresolver_hop * )from->data;
	pathresolver_hop *h_to = ( pathresolver_hop * )to->data;

	delete_hash_entry( dlist_table, &h_from->dpid );
	insert_hash_entry( dlist_table, &h_to->dpid, to );
}

static dlist_element *
create_routing_list( hash_table *dlist_table, uint64_t *dpid) {
	return ( dlist_element * )lookup_hash_entry( dlist_table, dpid );
}

static dlist_element *
find_dlist( hash_table *dlist_table, node *n ) {
	return ( dlist_element * )lookup_hash_entry( dlist_table, &n->dpid );
}

static dlist_element *
WSP_routing( hash_table *node_table, uint64_t in_dpid, uint16_t in_port_no,
						 uint64_t out_dpid, uint16_t out_port_no ) {
	int H = -1;		// hop count is infinite.

	node *src_node = lookup_node( node_table, in_dpid );
	node *dst_node = lookup_node( node_table, out_dpid );

	if( src_node == NULL )	return NULL;
	if( dst_node == NULL )	return NULL;

	hash_table *dlist_table = create_dlist_table();

	dlist_element *h = create_dlist();
	pathresolver_hop *hop = xmalloc( sizeof( pathresolver_hop ) );
	hop->dpid = in_dpid;
	hop->in_port_no = in_port_no;
	h->data = hop;

	update( dlist_table, h, h );

	hash_iterator iter_dlist;
	hash_iterator iter_edge;
	hash_entry *entry_dlist = NULL;
	hash_entry *entry_edge = NULL;
	dlist_element *cur_dlist = NULL;
	dlist_element *old_dlist = NULL;
	dlist_element *new_dlist = NULL;
	edge *e = NULL;
	node *n;
	node *candidate;
	pathresolver_hop *cur_hop;
	...
	for( i = 0 ; i < H ; i++ ) {
		init_hash_iterator( dlist_table, &iter_dlist );

		while( ( entry_dlist = iterate_hash_next( &iter_dlist ) ) != NULL ) {
			cur_dlist = ( dlist_element * )entry_dlist->value;

			cur_hop = ( pathresolver_hop * )cur_dlist->data;

			n = lookup_node( node_table, cur_hop->dpid );

			init_hash_iterator( n->edges, &iter_edge );
			
			while( ( entry_edge = iterate_hash_next( &iter_edge ) ) != NULL ) {
				e = ( edge * )entry_edge->value;
				
				candidate = lookup_node( node_table, e->peer_dpid);
				
				if( candidate != NULL ) {
					new_dlist = create_dlist();
					pathresolver_hop *new_hop = xmalloc( sizeof( pathresolver_hop ) );
					
					new_hop->dpid = e->peer_dpid;
					new_hop->in_port_no = e->peer_port_no;

					new_dlist->data = new_hop;
					new_dlist->prev = cur_dlist;

					if( candidate->visited ) {
							// There are several route from this node.
							old_dlist = find_dlist( dlist_table, candidate );
							if( find_min_r( node_table, old_dlist ) < find_min_r( node_table, new_dlist ) )
								update( dlist_table, old_dlist, new_dlist );
					} else {
						candidate->visited = true;
						
						update( dlist_table, cur_dlist, new_dlist );
					}

					// if candidate node is dst node then flag on to any route can be intend route other toutes.
					if( candidate == dst_node )	flag = true;
				}
			}
		}

		if( flag )	return find_dlist( dlist_table, dst_node );
	}

	return NULL; // Not found a route to dst from src with H hops.
}
